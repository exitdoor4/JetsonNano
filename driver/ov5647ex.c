...
#include <media/v4l2-ctrls.h> // o
#include <media/v4l2-device.h> // o
#include <media/v4l2-event.h> // o
#include <media/v4l2-of.h>  // x
#include <media/v4l2-image-sizes.h> // o
		      ...
		      unsigned int pad, enum v4l2_subdev_format_whence which)
{
	...
	case V4L2_SUBDEV_FORMAT_TRY:
		return v4l2_subdev_get_try_crop(sd_state, pad); //d 
...
static int ov5647_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable) {
		ret = pm_runtime_resume(&client->dev); // o
		if (ret < 0)
			goto error_unlock;
...
static int ov5647_init_controls(struct ov5647 *sensor)
{
	...
	v4l2_ctrl_handler_init(&sensor->ctrls, 9); // d
